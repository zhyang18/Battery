...
| 🔋 系统 API 检测 | ⚡ Shizuku 提权检测 | 📈 历史趋势与衰减统计 | ⚙️ 设置与数据备份 |
| :---: | :---: | :---: | :---: |
| <img src="screenshots/01_detection_api.png" width="220" alt="系统API检测" /> | <img src="screenshots/02_detection_shizuku.png" width="220" alt="Shizuku提权检测" /> | <img src="screenshots/03_history_trend.png" width="220" alt="历史趋势与衰减统计" /> | <img src="screenshots/04_settings.png" width="220" alt="设置与数据备份" /> |
...
> [!WARNING]
> **⚠️ 机型适配说明**：
> 目前 **Shizuku 提权读取** 与 **错误报告 (Bugreport) 解析** 方式**主要针对荣耀（HONOR）手机进行了深度适配**，其它品牌手机（如小米、OPPO、vivo、三星等）暂未充分测试，部分私有底层驱动节点与错误报告格式可能存在兼容性差异。系统标准 API 检测支持所有通用 Android 设备。

---

### 2. 📈 电池健康度衰减趋势与日/月/年损耗统计
* **平滑贝塞尔健康度趋势图**：基于 Canvas 硬件加速拟合历史快照曲线，支持手势滑动探查每个快照节点详情。
* **全维度衰减速率推算**：
  - 📅 **每日平均衰减值 (%/天)**；
  - 📆 **每月平均衰减值 (%/月)**；
  - 🗓️ **每年预估衰减值 (%/年)**。
* **周期实际损耗明细**：支持按自然日、自然月、自然年自动分组聚合，详细查看每个周期的起止健康度变化与充放电循环次数增量。
